...
public class TPTextureAtlas
{
    public TPTextureAtlas()
    {
        Regions = new Dictionary<string, TPTextureRegion>();
    }
    ...
    public void BuildXML(String resourceName)
    {
        ...
        foreach (XmlNode spriteNode in nodes)
        {
            ...
            TPTextureRegion textureRegion = new TPTextureRegion();
    ...
    public void BuildTPSheet(String resourceName, Texture2D texture)
    {
        TextAsset textAsset = Resources.Load<TextAsset>(resourceName);
        var uvx = 1f / texture.width;
        var uvy = 1f / texture.height;
        ...
        foreach (string line in allLines)
        {
            ...
            if (lineTokens.Length == 7)
            {
                ...
                var frameRect = extractUVRect(lineTokens, texture);
                var spriteSize = new Vector2(frameRect.width / uvx, frameRect.height / uvy);
    ...
    internal TPTextureAtlas(Dictionary<string, TPTextureRegion> regions)
    {
        Regions = regions;
    }
    ...
    public TPTextureRegion GetRegion(string textureName)
    {
        TPTextureRegion region;
    ...
    private static Rect extractUVRect(String[] data, Texture2D atlas)
    {
        ...
        var w = float.Parse(data[3]);
        var h = float.Parse(data[4]);

        //Rect r = new Rect(x * uvx, 1f - ((y) * uvy), (x + w) * uvx, 1f - ((y + h) * uvy));
        //Rect r = new Rect(x * uvx, ((y) * uvy), (x + w) * uvx, ((y + h) * uvy));
        Rect r = new Rect(x * uvx, ((y) * uvy), (x + w) * uvx, ((y + h) * uvy));
    ...
    Dictionary<string, TPTextureRegion> Regions { get; set; }
