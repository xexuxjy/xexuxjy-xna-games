...
public class SpriteManager : MonoBehaviour 
{
	...
	protected TexturePackerSprite[] sprites;				// Array of all sprites (the offset of the vertices corresponding to each sprite should be found simply by taking the sprite's index * 4 (4 verts per sprite).
	...
	protected TexturePackerSpriteDrawLayerComparer drawOrderComparer = new TexturePackerSpriteDrawLayerComparer(); // Used to sort our draw order array
	...
	// Working vars:
	protected int i;
	protected TexturePackerSprite tempSprite = null;
	...
	{
		sprites = new TexturePackerSprite[1];
		sprites[0] = new TexturePackerSprite();
	...
	protected int EnlargeArrays(int count)
	{
		...
		// Resize sprite array:
		TexturePackerSprite[] tempSprites = sprites;
		sprites = new TexturePackerSprite[sprites.Length + count];
		...
		for (int i = firstNewElement; i < sprites.Length; ++i)
		{
			...
			sprites[i] = new TexturePackerSprite();
			sprites[i].index = i;
	...
	public TexturePackerSprite AddSprite(GameObject client, float width, float height, int leftPixelX, int bottomPixelY, int pixelWidth, int pixelHeight, bool billboarded)
	{
	...
	public TexturePackerSprite AddSprite(GameObject client, float width, float height, int leftPixelX, int bottomPixelY, int pixelWidth, int pixelHeight, Vector3 offset, bool billboarded)
	{
	...
	// offset - the sprite will be offseted by the specified Vector3 relative to the client transform
	public TexturePackerSprite AddSprite(GameObject client, float width, float height, Vector2 lowerLeftUV, Vector2 UVDimensions, Vector3 offset, bool billboarded)
	{
		int spriteIndex;
		...
		spriteIndex = ((TexturePackerSprite)availableBlocks[0]).index;
		availableBlocks.RemoveAt(0);	// Now that we're using this one, remove it from the available list
 
		// Assign the new sprite:
		TexturePackerSprite newSprite = sprites[spriteIndex];
	...
	Vector3 CalculateNormal( TexturePackerSprite sprite )
	{
	...
	public void SetBillboarded(TexturePackerSprite sprite)
	{
	...
	public void RemoveSprite(TexturePackerSprite sprite)
	{
	...
	public void HideSprite(TexturePackerSprite sprite)
	{
		// Remove the sprite from the billboarded list
	...
	public void ShowSprite(TexturePackerSprite sprite)
	{
	...
	public void MoveToFront(TexturePackerSprite s)
	{
	...
	public void MoveToBack(TexturePackerSprite s)
	{
		int[] indices = new int[6];
		int offset = spriteDrawOrder.IndexOf(s) * 6;
	...
	public void MoveInfrontOf(TexturePackerSprite toMove, TexturePackerSprite reference)
	{
		int[] indices = new int[6];
	...
	public void MoveBehind(TexturePackerSprite toMove, TexturePackerSprite reference)
	{
		int[] indices = new int[6];
		int offset = spriteDrawOrder.IndexOf(toMove) * 6;
		int refOffset = spriteDrawOrder.IndexOf(reference) * 6;
	...
	public void SortDrawingOrder()
	{
		...
		if (winding == WINDING_ORDER.CCW)
		{
			...
			{
				s = (TexturePackerSprite) spriteDrawOrder[i];
	...
	public void AnimateSprite(TexturePackerSprite s)
	{
		// Add this sprite to our playingAnimation list:
		playingAnimations.Add(s);
	}
 
	public void StopAnimation(TexturePackerSprite s)
	{
		playingAnimations.Remove(s);
	}
 
	public TexturePackerSprite GetSprite(int i)
	{
		if (i < sprites.Length)
			return sprites[i];
		else
	...
	public void Transform(TexturePackerSprite sprite)
	{
		sprite.Transform();
	...
	public void TransformBillboarded(TexturePackerSprite sprite)
	{
	...
	public void UpdateUV(TexturePackerSprite sprite)
	{
	...
	public void UpdateColors(TexturePackerSprite sprite)
	{
	...
	virtual public void LateUpdate () 
	{
		...
		if(playingAnimations.Count > 0)
		{
			...
			{
				tempSprite = (TexturePackerSprite)playingAnimations[i];
