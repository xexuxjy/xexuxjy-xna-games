...
public class CombatEngine : MonoBehaviour
{
    ...
    {
        if (attacker == null || defender == null || attackSkill == null)
        {
            int ibreak = 0;
        }
        ...
        // practice props only have strong responses
        if (defender.ActorClass == ActorClass.DUMMY)
        {
            attackResult.isStrongAttack = true;
        }
    ...
    private void DrawCombatResult(AttackSkill attackSkill, AttackResult attackResult, BaseActor attacker, BaseActor defender)
    {
        ...
        CombatEngineUI.DrawFloatingText(defender.CharacterFocusPoint, color, text, 2f);
    ...
    public static bool IsNearDeath(BaseActor actor)
    {
        return ((float)actor.Health) / ((float)actor.MaxHealth) < 0.2f;
    }
    ...
    public bool IsAttackerInRange(BaseActor attacker, BaseActor defender, bool cursorOnly = false)
    {
        if (defender != null && attacker != null)
        {
            ...
            int len = (int)(Math.Abs(diff.x) + Math.Abs(diff.z));//(int)diff.magnitude;
            if (len == 1)
            {
                return true;
            }

            if (attacker.CurrentAttackSkill != null)
            {
    ...
    public void ApplySkill(AttackSkill skill, BaseActor attacker, BaseActor defender, AttackResult result)
    {
        switch (skill.m_skillEffect.Name)
        {
            ...
            case "Change Crowd Source":
                Crowd.UpdateSchoolScore(attacker.School, (int)skill.m_skillEffect.Modifier1);
                break;
            case "Change Crowd Target":
                Crowd.UpdateSchoolScore(defender.School, (int)skill.m_skillEffect.Modifier1);
                break;
            case "Avoid":
